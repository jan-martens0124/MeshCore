...
namespace Intersection {
    ...
    float intersectionDistance(const Ray& ray, const VertexTriangle& triangle) {
        ...
        //Möller–Trumbore
        glm::vec3 h = glm::cross(ray.direction, -triangle.edge2);

        float a = glm::dot(triangle.edge0, h);
        ...
        glm::vec3 q = glm::cross(s, triangle.edge0);
        float v = f * glm::dot(ray.direction, q);
        if (v < 0.0 || u + v > 1.0) {
            return - std::numeric_limits<float>::max(); // No line intersection
        }
        // At this stage we can compute t to find out where the intersection point is on the line.
        float t = f * glm::dot(-triangle.edge2, q);
        return t;
    }
    ...
    float intersectionDistanceJGT(const Ray& ray, const VertexTriangle& triangle){
        ...
        float t = glm::dot(-triangle.edge2, q) * f;
        return t;
    }
}
